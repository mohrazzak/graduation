"""Ultralytics adapter for the trained four-class building-damage detector.
...
def _load_yolo(path: str) -> Any:
    """Import Ultralytics lazily so the mock roster never pays for torch."""
...
class RaedClassifier:
    """Detect building regions and summarize them conservatively by severity."""

    # The id is persisted in Supabase rows and used for routing — it stays "raed".
    # Only the human-facing label differs.
    id = "raed"
    name = "Trained Model"
        ...
        *,
        ...
        self._confidence_threshold = (
            ...
            if confidence_threshold is not None
                ...
                os.environ.get(
                    "RAED_CONFIDENCE_THRESHOLD", str(DEFAULT_CONFIDENCE_THRESHOLD)
                )
            )
        )
